...
class StoreManager(object):
    ...
    def get_file_dict(self, _file):
        text = self.read_file(_file)
        self._logger.debug("Reading texts from {} {}".format(_file, text))

        file_dict = self._load_yaml_instance_from_text(text)

        return file_dict

    # def get_template(self, _template_file="playground.yaml"):
    #     text = self.read_file(_template_file)
    #     template = self._load_yaml_instance_from_text(text)
    #     return template
    #
    # def get_boxes(self, _box_file="box.yaml"):
    #     text = self.read_file(_box_file)
    #     boxes = self._load_yaml_instance_from_text(text)
    #     return boxes
