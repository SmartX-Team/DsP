...
class ProvisionCoordinator(object):
    ...
    def provisioning(self, playground):
        ...
        for box_with_softwares in self._playground:
            proc = Process(target=self._install_softwares_to_box,
                           args=(box_with_softwares, self._inventory, lock, result_queue,))
            procs.append(proc)
            proc.start()
    ...
    def _install_softwares_to_box(self, box_with_softwares, _inven_mgr, _lock, result_queue):
    ...
    def _get_installer(self, installer_name, installer_queue):
        for installer_instance in installer_queue:
